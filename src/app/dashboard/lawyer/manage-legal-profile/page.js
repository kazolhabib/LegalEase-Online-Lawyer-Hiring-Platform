...
const isAutoGeneratedDraftProfile = (profile) => (
  profile?.bio === AUTO_DRAFT_DEFAULTS.bio
  && profile?.specialization === AUTO_DRAFT_DEFAULTS.specialization
  && Number(profile?.rate) === AUTO_DRAFT_DEFAULTS.rate
  && profile?.status === AUTO_DRAFT_DEFAULTS.status
  && profile?.badge === AUTO_DRAFT_DEFAULTS.badge
  && typeof profile?.image === 'string'
  && profile.image.includes('images.unsplash.com/photo-1560250097-0b93528c311a')
);
...
export default function LawyerManageProfilePage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (myProfile) {
          ...
          const draft = isAutoGeneratedDraftProfile(myProfile)
            ...
            : {
                ...
                isPublished: myProfile.isPublished ?? false
              };
          setBio(draft.bio);
          setSpecialization(draft.specialization);
          setRate(draft.rate);
          ...
          const draft = getEmptyListingDraft();
          setBio(draft.bio);
          setSpecialization(draft.specialization);
          setRate(draft.rate);
          setImage(draft.image);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!specialization || rate === '' || !image.trim() || !status || !badge || !bio.trim()) {
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-[1rem] bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-[30rem] rounded-[1.5rem] border border-accent/20 bg-card dark:bg-zinc-950 p-[2rem] text-center shadow-[0_2rem_5rem_rgba(0,0,0,0.35)]">
            ...
            <span className="text-[0.625rem] uppercase tracking-[0.25em] text-accent font-extrabold">
              {isPublished ? 'Listing Published' : 'Profile Saved'}
            </span>
            ...
            <p className="mt-[0.75rem] text-[0.8125rem] leading-relaxed text-slate-500">
              {isPublished
                ...
                : 'Your profile details are saved as a draft. Turn on Publish Listing when you are ready for clients to discover it.'}
            </p>
            ...
            <div className="mt-[1.75rem] grid grid-cols-1 sm:grid-cols-2 gap-[0.75rem]">
              ...
              {isPublished ? (
                ...
                <button
                  type="button"
                  onClick={() => router.push('/dashboard')}
                  className="rounded-[0.75rem] bg-primary px-[1rem] py-[0.75rem] text-[0.75rem] font-bold text-white hover:scale-[1.01] transition-all dark:bg-accent dark:text-navy cursor-pointer"
                >
                  Back to Dashboard
                </button>
              )}
